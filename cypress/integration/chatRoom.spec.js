...
describe("Chat room messages and room change", () => {
      ...
      cy.fixture("rooms").then((json) => {
        ...
        rooms.forEach((room) => {
          ...
          messages.forEach((message) => {
            ...
            cy.callFirestore("set", `rooms/${room.uid}/messages/${message.uid}`, message);
          });
        });
      });
    ...
    it("Choose a chat room and check room name and messages load", () => {
      ...
      cy.get('[data-testid="room-name"]').should("have.text", testUser1DisplayName);
    ...
    it("Load and exchange messages with two different users", () => {
      ...
      cy.callFirestore("set", `rooms/${rooms[0].uid}/messages/${messageToSendFromTestUser1.uid}`, messageToSendFromTestUser1);
      ...
      const messageFromLoginUserToTestUser1 = messagesToSendFromLoginUser[0].message;
      ...
      cy.get('[data-testid="room-name"]').should("have.text", testUser2DisplayName);
      // "Send" a message from test user 2 and check that it renders
      cy.callFirestore("set", `rooms/${rooms[1].uid}/messages/${messageToSendFromTestUser2.uid}`, messageToSendFromTestUser2);
      cy.contains(messageToSendFromTestUser2.message);
      // Send a message from the current user and check that the form clears and sent message renders
      const messageFromLoginUserToTestUser2 = messagesToSendFromLoginUser[1].message;
